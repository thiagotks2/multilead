...
The setup of this project was optimized using raw **Docker** via the `docker-compose.yml` in the root — ensuring freedom of control.

### How to Run the Project in DEV environment (Step-by-step)
...
4. **Work via SSH in the Containers:**
   Use the Alpine *shell* (`ash`) to primarily enter the main application host where we will perform command-line actions:
   ```bash
...
> **Standardization Maintenance (Pint):**
